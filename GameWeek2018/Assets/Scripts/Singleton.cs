...
public abstract class Singleton<T> : MonoBehaviour where T : MonoBehaviour
{
    ...
    private static void CreateInstance()
    {
        ...
        if (objects.Length > 0)
        {
            if (objects.Length > 1)
            {
                Debug.LogWarning("<color=red>There is more than one instance of Singleton of type \"" + type +
                                 "\". Keeping the first one. Destroying the others.</color>");
        ...
        _instance           =   _instance ?? gameObject.GetComponent<T>() ?? gameObject.AddComponent<T>();
        Instantiated        =   true;
        Destroyed           =   false;
    }
